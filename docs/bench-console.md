...
| `a` | activate the default app (suspend/switch/resume, same path as the app menu; for headless benches where boot recall restored a different app) |
| `I` | toggle app ISR |
...
### DANGER — destructive (second press within 3 s required)
| key | action |
|---|---|
| `C` | **clear/reset the config file** (GLOBALS.CFG rewritten empty) |
| `F` | **erase ALL LittleFS files** — presets, config, CRASH.LOG, everything |

Both keys arm on the first press and only fire on a second press of the
same key within 3 seconds (`pew!` stops robots typing garbage; this
stops human typos).
...
```
=== selftest ===
uptime=123s  reset_cause(SRC_SRSR@boot)=00000001
watchdog: armed (128s, fed from loop)
loop rate ~4200 Hz
core delta5ms=83 (expect ~83)
heap free: 380000 bytes (RAM2)
audio pool: 4 now / 9 max   cpu: 8.4% now / 12.1% max
...
- **reset_cause** — raw `SRC_SRSR` hex, captured then cleared at boot so
  each boot reports only its own cause. A ` [WDOG]` suffix means the last
...
- **audio pool / cpu** — audio block pool, current/high-water, plus
  audio CPU now/peak. A pool `max` at the pool ceiling means allocation
  failures have likely occurred.
...
1. **Wedged loop()** — the hardware watchdog (WDOG1, 128 s) reboots the
   module instead of leaving it bricked until power-cycle. The timeout
   clears every legitimate blocking op (a console `F` full format, whose
   worst case can exceed it, runs under a temporary 1 Hz timer feeder).
   After the reboot, boot recall restores
