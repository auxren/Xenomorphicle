...
// AudioEffectModalResonatorF32 — parallel bank of modal resonators, float32
//
// Float32 port of AudioEffectModalResonator (see AudioEffectModalResonator.h).
// The int16 original already ran its 12-mode Chamberlin SVF bank in float
// internally; the only int16 stage was the per-sample output quantisation
// (Clip16(y * 32767)), which truncated low-amplitude ring tails — exactly the
// signal a high-Q modal bank is about. This version keeps the audio blocks
// float32 end to end: the decaying tails fade below -96 dBFS smoothly instead
// of collapsing into the int16 quantisation floor, and the wet path reaches
// the wet/dry mixer unclipped at full precision.
//
// Bit-compatibility with the int16 engine:
//   - updateCoeffs() is copied verbatim — identical coefficients for
//     identical parameters (control-rate math unchanged).
//   - The excitation path is bit-exact: the I16→F32 edge adapter divides by
//     32768 (exact power of two) and the 0.125 input scale is also a power of
//     two, so x matches the original's (int16 * 0.125f/32768.0f) exactly; the
//     xorshift strike-noise sequence and scaling are unchanged.
//   - VU peak readback keeps the same uint16 scale and decay behaviour.
//
// Filter topology: Chamberlin SVF (see the int16 header for the full notes on
// the stiffness model, Q model, stability clamps, and denormal handling).
//
// Thread safety: aligned 32-bit float writes from Controller() are naturally
// atomic on Cortex-M7.
...
class AudioEffectModalResonatorF32 : public AudioStream_F32 {
public:
    static constexpr int NUM_MODES = 12;

    AudioEffectModalResonatorF32() : AudioStream_F32(1, input_queue_array_) {}
    ...
    // Verbatim from the int16 engine — see AudioEffectModalResonator.h for the
    // derivation of the Q range, stiffness model, and stability clamps.
    ...
    void update() override {
        ...
        const float* src = in ? in->data : nullptr;
        float*       dst = out->data;
        ...
        for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++) {
            ...
            // SVF state saturation on loud signals. F32 blocks are ±1.0
            // full-scale, so this matches the int16 engine's
            // (int16 * 0.125f/32768.0f) bit-exactly.
            ...
            dc_x1_ = y;
            dc_y1_ = y_out;
            // Full-precision float out — no Clip16 quantisation of ring tails.
            // (The F32→I16 edge adapter saturates at the applet boundary.)
            dst[i] = y_out;
            // Track output peak on the same uint16 VU scale as the int16 engine
            {
                float ay = y_out < 0.0f ? -y_out : y_out;
                uint32_t av = (uint32_t)(ay * 32767.0f);
                if (av > 32767) av = 32767;
                if (av > output_peak_) output_peak_ = (uint16_t)av;
            }
        }
